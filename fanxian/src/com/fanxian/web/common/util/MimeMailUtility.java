...
/**
 * 格式化邮件用的<br />
 * 这个类其实就是javamail里MimeUtility里复制过来的,里面几个方法改造了一下
 */
public class MimeMailUtility {
    ...
    /**
     * Get the content-transfer-encoding that should be applied to the input stream of this datasource, to make it
     * mailsafe.
     * <p>
     * The algorithm used here is: <br>
     * <ul>
     * <li>If the primary type of this datasource is "text" and if all the bytes in its input stream are US-ASCII, then
     * the encoding is "7bit". If more than half of the bytes are non-US-ASCII, then the encoding is "base64". If less
     * than half of the bytes are non-US-ASCII, then the encoding is "quoted-printable".
     * <li>If the primary type of this datasource is not "text", then if all the bytes of its input stream are US-ASCII,
     * the encoding is "7bit". If there is even one non-US-ASCII character, the encoding is "base64".
     * </ul>
     * 
     * @param ds DataSource
        ...
        try {
            ...
            switch (i) {
                case ALL_ASCII:
                    ...
                    break;
                case MOSTLY_ASCII:
                    if (isText && nonAsciiCharset(cType)) encoding = "base64"; // charset isn't compatible with ASCII
                    else encoding = "quoted-printable"; // mostly ASCII
                    break;
                default:
    ...
    /**
     * Determine whether the charset in the Content-Type is compatible with ASCII or not. A charset is compatible with
     * ASCII if the encoded byte stream representing the Unicode string "\r\n" is the ASCII characters CR and LF. For
     * example, the utf-16be charset is not compatible with ASCII. For performance, we keep a static map that caches the
     * results.
     */
    ...
    /**
     * Same as <code>getEncoding(DataSource)</code> except that instead of reading the data from an
     * <code>InputStream</code> it uses the <code>writeTo</code> method to examine the data. This is more efficient in
     * the common case of a <code>DataHandler</code> created with an object and a MIME type (for example, a "text/plain"
     * String) because all the I/O is done in this thread. In the case requiring an <code>InputStream</code> the
     * <code>DataHandler</code> uses a thread, a pair of pipe streams, and the <code>writeTo</code> method to produce
     * the data.
     * <p>
     * 
     * @since JavaMail 1.2
    ...
    public static String getEncoding(DataHandler dh) {
        ...
        /*
         * Try to pick the most efficient means of determining the encoding. If this DataHandler was created using a
         * DataSource, the getEncoding(DataSource) method is typically faster. If the DataHandler was created with an
         * object, this method is much faster. To distinguish the two cases, we use a heuristic. A DataHandler created
         * with an object will always have a null name. A DataHandler created with a DataSource will usually have a
         * non-null name. XXX - This is actually quite a disgusting hack, but it makes a common case run over twice as
         * fast.
         */
    ...
    /**
     * Decode the given input stream. The Input stream returned is the decoded input stream. All the encodings defined
     * in RFC 2045 are supported here. They include "base64", "quoted-printable", "7bit", "8bit", and "binary". In
     * addition, "uuencode" is also supported.
     * <p>
     * In the current implementation, if the <code>mail.mime.ignoreunknownencoding</code> system property is set to
     * <code>"true"</code>, unknown encoding values are ignored and the original InputStream is returned.
     * 
    ...
    public static InputStream decode(InputStream is, String encoding) throws MessagingException {
        if (encoding.equalsIgnoreCase("base64")) return new BASE64DecoderStream(is);
            ...
            if (!ignoreUnknownEncoding) throw new MessagingException("Unknown encoding: " + encoding);
            return is;
        }
    }

    /**
     * Wrap an encoder around the given output stream. All the encodings defined in RFC 2045 are supported here. They
     * include "base64", "quoted-printable", "7bit", "8bit" and "binary". In addition, "uuencode" is also supported.
     * 
    ...
    /**
     * Wrap an encoder around the given output stream. All the encodings defined in RFC 2045 are supported here. They
     * include "base64", "quoted-printable", "7bit", "8bit" and "binary". In addition, "uuencode" is also supported. The
     * <code>filename</code> parameter is used with the "uuencode" encoding and is included in the encoded output.
     * 
     * @param os output stream
     * @param encoding the encoding of the stream.
     ...
     * <p>
     * The given Unicode string is examined for non US-ASCII characters. If the string contains only US-ASCII
     * characters, it is returned as-is. If the string contains non US-ASCII characters, it is first character-encoded
     * using the platform's default charset, then transfer-encoded using either the B or Q encoding. The resulting bytes
     * are then returned as a Unicode string containing only ASCII characters.
     * <p>
     ...
     * The given Unicode string is examined for non US-ASCII characters. If the string contains only US-ASCII
     * characters, it is returned as-is. If the string contains non US-ASCII characters, it is first character-encoded
     * using the specified charset, then transfer-encoded using either the B or Q encoding. The resulting bytes are then
     * returned as a Unicode string containing only ASCII characters.
     * <p>
     * Note that this method should be used to encode only "unstructured" RFC 822 headers.
     * 
     * @param text the header value
     * @param charset the charset. If this parameter is null, the platform's default chatset is used.
     * @param encoding the encoding to be used. Currently supported values are "B" and "Q". If this parameter is null,
     * then the "Q" encoding is used if most of characters to be encoded are in the ASCII charset, otherwise "B"
     * encoding is used.
     * @return Unicode string containing only US-ASCII characters
     */
     ...
     * <p>
     * The string is decoded using the algorithm specified in RFC 2047, Section 6.1. If the charset-conversion fails for
     * any sequence, an UnsupportedEncodingException is thrown. If the String is not an RFC 2047 style encoded header,
    ...
    public static String decodeText(String etext) throws UnsupportedEncodingException {
        /*
         * We look for sequences separated by "linear-white-space". (as per RFC 2047, Section 6.1) RFC 822 defines
         * "linear-white-space" as SPACE | HT | CR | NL.
         */
        String lwsp = " \t\n\r";
        StringTokenizer st;

        /*
         * First, lets do a quick run thru the string and check whether the sequence "=?" exists at all. If none exists,
         * we know there are no encoded-words in here and we can just return the string as-is, without suffering thru
         * the later decoding logic. This handles the most common case of unencoded headers efficiently.
         */
     ...
     * are then returned as a Unicode string containing only ASCII characters.
     * <p>
     * This method is meant to be used when creating RFC 822 "phrases". The InternetAddress class, for example, uses
     * this to encode it's 'phrase' component.
     * 
    ...
    /*
     * Encode the given string. The parameter 'encodingWord' should be true if a RFC 822 "word" token is being encoded
     * and false if a RFC 822 "text" token is being encoded. This is because the "Q" encoding defined in RFC 2047 has
    ...
    private static String encodeWord(String string, String charset, String encoding, boolean encodingWord)
        ...
        else throw new UnsupportedEncodingException("Unknown transfer encoding: " + encoding);
    ...
    /**
     * The string is parsed using the rules in RFC 2047 and RFC 2231 for parsing an "encoded-word". If the parse fails,
     * a ParseException is thrown. Otherwise, it is transfer-decoded, and then charset-converted into Unicode. If the
     * charset-conversion fails, an UnsupportedEncodingException is thrown.
     * <p>
     * 
     * @param eword the encoded value
     * @exception ParseException if the string is not an encoded-word as per RFC 2047 and RFC 2231.
     * @exception UnsupportedEncodingException if the charset conversion failed.
     */
    public static String decodeWord(String eword) throws ParseException, UnsupportedEncodingException {
        ...
        int start = 2;
        int pos;
        if ((pos = eword.indexOf('?', start)) == -1) throw new ParseException("encoded word does not include charset: "
                                                                              + eword);
        ...
        if ((pos = eword.indexOf("?=", start)) == -1) throw new ParseException(
                                                                               "encoded word does not end with \"?=\": "
                                                                                       + eword);
        /*
         * XXX - should include this, but leaving it out for compatibility... if (decodeStrict && pos != eword.length()
         * - 2) throw new ParseException( "encoded word does not end with \"?=\": " + eword););
         */
        ...
        try {
            ...
            if (word.length() > 0) {
                ...
                else throw new UnsupportedEncodingException("unknown encoding: " + encoding);
            ...
            /*
             * An unknown charset of the form ISO-XXX-XXX, will cause the JDK to throw an IllegalArgumentException ...
             * Since the JDK will attempt to create a classname using this string, but valid classnames must not contain
             * the character '-', and this results in an IllegalArgumentException, rather than the expected
    ...
    public static HashMap decodeWordTags(String eword) throws ParseException, UnsupportedEncodingException {
        HashMap info = new HashMap();
        eword = MimeMailUtility.unfold(eword);
        if (!eword.startsWith("=?")) {
            eword = new String(ASCIIUtility.getBytes(eword), "GBK");
            info.put("word", eword);
            // System.out.println("encoded word does not start with \"=?\": " + eword);
            return info;
        }
        ...
        // if(pos == -1)
        // {
        // System.out.println("encoded word does not end with \"?=\": " + eword);
        // }
        /*
         * XXX - should include this, but leaving it out for compatibility... if (decodeStrict && pos != eword.length()
         * - 2) throw new ParseException( "encoded word does not end with \"?=\": " + eword););
         */
        String word = pos == -1 ? eword.substring(start) : eword.substring(start, pos);
        info.put("word", word);
        info.put("wordstart", start);
        info.put("wordpos", pos);
        if (pos != -1 && pos + 2 < eword.length()) {
            // there's still more text in the string
            String rest = eword.substring(pos + 2);
            if (!rest.equalsIgnoreCase(" ") && !rest.equalsIgnoreCase("\r\n") && !rest.equalsIgnoreCase("\r\n\t")) {
                info.put("rest", rest);
            }
        }
        return info;
    ...
    public static String decodeWord(String word, String encoding, String charset, String rest) {
        try {
            String decodedWord;
            if (word.length() > 0 && encoding != null) {
                // Extract the bytes from word
                ByteArrayInputStream bis = new ByteArrayInputStream(ASCIIUtility.getBytes(word));
            ...
            /*
             * if (pos + 2 < eword.length()) { // there's still more text in the string String rest =
             * eword.substring(pos + 2);
             */
            if (rest != null) {
                if (!decodeStrict) rest = decodeInnerWords(rest);
                decodedWord += rest;
    ...
    /**
     * Look for encoded words within a word. The MIME spec doesn't allow this, but many broken mailers, especially
     * Japanese mailers, produce such incorrect encodings.
     */
     ...
     * <p>
     * The <code>HeaderTokenizer</code> class defines two special sets of delimiters - MIME and RFC 822.
     * <p>
    ...
    /**
     * Fold a string at linear whitespace so that each line is no longer than 76 characters, if possible. If there are
     * more than 76 non-whitespace characters consecutively, the string is folded at the first whitespace after that
     * sequence. The parameter <code>used</code> indicates how many characters have been used in the current line; it is
     * usually the length of the header name.
     * <p>
    ...
    /**
     * Return the first index of any of the characters in "any" in "s", or -1 if none are found. This should be a method
     * on String.
     */
     ...
     * @return the Java charset equivalent. If a suitable mapping is not available, the passed in charset is itself
     * returned.
     */
     ...
     * <p>
     * Note that a future version of JDK (post 1.2) might provide this functionality, in which case, we may deprecate
     * this method then.
     * 
     * @param charset the JDK charset
     * @return the MIME/IANA equivalent. If a mapping is not possible, the passed in charset itself is returned.
     * @since JavaMail 1.1
     */
    ...
    /**
     * Get the default charset corresponding to the system's current default locale. If the System property
     * <code>mail.mime.charset</code> is set, a system charset corresponding to this MIME charset will be returned.
     * <p>
     * 
     ...
     * 
     * @param s string
     * @return ALL_ASCII if all characters in the string belong to the US-ASCII charset. MOSTLY_ASCII if more than half
     * of the available characters are US-ASCII characters. Else MOSTLY_NONASCII.
     */
     ...
     * @param b byte array
     * @return ALL_ASCII if all characters in the string belong to the US-ASCII charset. MOSTLY_ASCII if more than half
     * of the available characters are US-ASCII characters. Else MOSTLY_NONASCII. XXX - this method is no longer used
     */
    ...
    /**
     * Check if the given input stream contains non US-ASCII characters. Upto <code>max</code> bytes are checked. If
     * <code>max</code> is set to <code>ALL</code>, then all the bytes available in this input stream are checked. If
     * <code>breakOnNonAscii</code> is true the check terminates when the first non-US-ASCII character is found and
     * MOSTLY_NONASCII is returned. Else, the check continues till <code>max</code> bytes or till the end of stream.
     * 
     * @param is the input stream
     * @param max maximum bytes to check for. The special value ALL indicates that all the bytes in this input stream
     * must be checked.
     * @param breakOnNonAscii if <code>true</code>, then terminate the the check when the first non-US-ASCII character
     * is found.
     * @return ALL_ASCII if all characters in the string belong to the US-ASCII charset. MOSTLY_ASCII if more than half
     * of the available characters are US-ASCII characters. Else MOSTLY_NONASCII.
     */
    ...
    public static String gb2312togbk(String gb2312) {
        if (gb2312.equalsIgnoreCase("gb2312")) {
            return "GBK";
        }
        return gb2312;
    }
...
class AsciiOutputStream extends OutputStream {
    ...
    private final void check(int b) throws IOException {
        ...
        if (MimeMailUtility.nonascii(b)) { // non-ascii
            non_ascii++;
            if (breakOnNonAscii) { // we are done
                ret = MimeMailUtility.MOSTLY_NONASCII;
    ...
    public int getAscii() {
        ...
        // will be base64 encoded (since the quoted-printable encoder
        // doesn't encode this case properly).
        if (badEOL) return MimeMailUtility.MOSTLY_NONASCII;
        else if (non_ascii == 0) { // no non-us-ascii characters so far
            // if we've seen a long line, we degrade to mostly ascii
            if (longLine) return MimeMailUtility.MOSTLY_ASCII;
            else return MimeMailUtility.ALL_ASCII;
        }
        if (ascii > non_ascii) // mostly ascii
        return MimeMailUtility.MOSTLY_ASCII;
        return MimeMailUtility.MOSTLY_NONASCII;
    }
}
