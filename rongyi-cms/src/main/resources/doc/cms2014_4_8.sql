...
CREATE TABLE `advertisements` (
  ...
  `delete_status` tinyint(4) DEFAULT NULL COMMENT '删除状态 0不删除 1删除',
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=17 DEFAULT CHARSET=utf8 COMMENT='广告详情';
...
-- ----------------------------
-- Table structure for cate_link_brands
-- ----------------------------
DROP TABLE IF EXISTS `cate_link_brands`;
CREATE TABLE `cate_link_brands` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `categories_id` varchar(32) DEFAULT NULL COMMENT '分类id',
  `brands_id` varchar(32) DEFAULT NULL COMMENT '品牌id',
  PRIMARY KEY (`id`)
...
CREATE TABLE `categories` (
  ...
  `prarent_id` varchar(32) DEFAULT '-1' COMMENT '是否为一级分类。-1为一级。二级填写一级id',
  `link_brands` int(11) DEFAULT NULL COMMENT '关联表 关联店铺 cate_link_shops',
  PRIMARY KEY (`id`)
...
CREATE TABLE `param` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=utf8 COMMENT='常量表';
...
CREATE TABLE `shops` (
  ...
  `shop_number` varchar(32) DEFAULT NULL COMMENT '商铺号 重点',
