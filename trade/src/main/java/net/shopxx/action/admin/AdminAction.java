...
public class AdminAction extends BaseAdminAction {
	...
	public static final String SPRING_SECURITY_LAST_EXCEPTION = "SPRING_SECURITY_LAST_EXCEPTION";// Spring security 登录异常Session名称
			...
			if (springSecurityLastException instanceof BadCredentialsException) {
				...
				if (admin != null) {
					...
					if (getSystemConfig().getIsLoginFailureLock() && loginFailureLockCount - loginFailureCount <= 3) {
						addActionError("若连续"+ loginFailureLockCount + "验证码输入错误您的账号将被锁定!");
					} else {
	...
	@Validations(
		...
		requiredFields = {
			@RequiredFieldValidator(fieldName = "admin.isAccountEnabled", message = "是否启用不允许为")
		},
		stringLengthFields = {
			@StringLengthFieldValidator(fieldName = "admin.username", minLength = "2", maxLength = "20", message = "用户名长度必须在${minLength}和{maxLength}之间!"),
			@StringLengthFieldValidator(fieldName = "admin.password", minLength = "4", maxLength = "20", message = "密码长度必须{minLength}和{maxLength}之间!")
		},
		emails = {
