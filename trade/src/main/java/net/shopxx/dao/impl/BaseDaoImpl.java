 ...
 * ---------------------------------------------------------------------------- 。
 * ----------------------------------------------------------------------------
 * ---------------------------------------------------------------------------- CED32589174C8D5905B5D0A02A2F33DF
 * ============================================================================
 */
...
public class BaseDaoImpl<T, PK extends Serializable> implements BaseDao<T, PK> {
    ...
    public Pager findByPager(Pager pager, DetachedCriteria detachedCriteria) {
        ...
        if (StringUtils.isNotEmpty(property) && StringUtils.isNotEmpty(keyword)) {
            ...
            // 略修改,适应integer 类型查询
            Class propertyClazz = getFiledType(entityClass, property);
            if (StringUtils.equals(propertyClazz.getSimpleName(), "Integer")) {
                criteria.add(Restrictions.eq(propertyString, new Integer(keyword)));
            } else {
                criteria.add(Restrictions.like(propertyString, "%" + keyword + "%"));
            }
        }

        Integer totalCount = Integer.parseInt(criteria.setProjection(Projections.rowCount()).uniqueResult() + "");
     ...
     * @return
     */
    private Class getFiledType(Class clazz, String fieldName) {
        try {
