...
<script type="text/javascript">
    $(document).ready(function(){
	  $("#delete").click(function() {	
		  many_deleterow("/crm/deleteVisit.action?seleteIDs=");
	  });	

	  $("#massUpdate").click(function() {	
		  many_massUpdaterow("/crm/editVisit.action?seleteIDs=");
	  });
	  
	  $("#export").click(function() {	
		  many_exportrow("/crm/exportVisit.action?seleteIDs=");
	  });

	  $("#copy").click(function() {	
		  many_copyrow("/crm/copyVisit.action?seleteIDs=");
	  });

	  var mygrid = jQuery("#grid").jqGrid({
			datatype: "json", 
			url:'listVisitFull.action', 
			mtype: 'POST',
			height: "auto",
		   	colNames:['<s:text name="entity.id.label" />','<s:text name="entity.subject.label" />',
		  		   	...
		  		   	'<s:text name="entity.end_date.label" />','<s:text name="visit.location.label" />',
		   	...
		   	rowList:[15,50,100], 
		   	multiselect: true, 
		   	caption: "<s:text name='title.grid.visits'/>"
		});
		function urlFmatter (cellvalue, options, rowObject)
		{  
		   var par='<%=((User)session.getAttribute("loginUser")).getUpdate_visit()%>';
		   if (par == 1){
			   new_format_value = "<a href='editVisit.action?id=" + rowObject[0] + "'>" + cellvalue + "</a>";
		   }else {
      ...
      <div id="shortcuts" class="headerList">
        ...
        <span style="white-space: nowrap;"> <a
            ...
            <div id="mtm1" style="width: 150px;">
              <s:if
                test="#session.loginUser.create_Visit == 1 || #session.loginUser.update_Visit == 1">
                <div data-options="iconCls:'icon-import'"
                  onClick="openwindow('/crm/upload.jsp?entityName=Visit&namespace=crm&title=' + '<s:text name="title.import.Visit" />')">
              ...
              <s:if test="#session.loginUser.update_Visit == 1">
                <div data-options="iconCls:'icon-update'"
                  id="massUpdate">
                  <s:text name='menu.item.massupdate.title' />
                </div>
              </s:if>
              <s:if test="#session.loginUser.create_Visit == 1">
